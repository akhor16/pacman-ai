...
def enhancedFeatureExtractorDigit(datum):
    ...
    ## DESCRIBE YOUR ENHANCED FEATURES HERE...
        computing gradients, computing distances from left and right borders for every height y
    ##
    """
    ...
    for x in range(DIGIT_DATUM_WIDTH - 1):
        for y in range(DIGIT_DATUM_HEIGHT - 1):
            features[("h", x, y)] = int(datum.getPixel(x, y) <
                                            datum.getPixel(x + 1, y))

            features[("v", x, y)] = int(datum.getPixel(x, y) <
                                            datum.getPixel(x, y + 1))
    # computing distances from left and right borders for every height y
    # iterate over every height y
    for y in range(DIGIT_DATUM_HEIGHT):
        # boolean variable if there is at least one dot on current y height
        hasX = False
        # iterate over X width from left to right
        for x in range(DIGIT_DATUM_WIDTH):
            # check if there is dot in x,y position
            if(datum.getPixel(x, y)):
                #we found first dot from the left
                features[("x1", y)] = x
                hasX = True
                break
        #iterate over X width from right to left
        for x in range(DIGIT_DATUM_WIDTH):
            # check if there is dot in x,y position
            if(datum.getPixel(DIGIT_DATUM_WIDTH - x - 1, y)):
                #we found first dot from the right
                features[("x2", y)] = DIGIT_DATUM_WIDTH - x - 1
                hasX = True
                break
        if not hasX:
            # zeroes if there is no dot for y height
            features[("x1", y)] = 0
            features[("x2", y)] = 0
...
def enhancedPacmanFeatures(state, action):
    ...
    my friend helped me with this features
    """
    # get variables
    ...
    #current distance to closest food
    minF = 11111111
    #iterate over foods
    for food in foods:
        #manhattan distance to current food
        cur = util.manhattanDistance(food, pac)
        #update minF if needed
        if cur < minF:
            minF = cur
    #add food feature
    features["food"] = 1.0 / minF

    #closest ghost current variable
    minG = 1111111111111
    #iterate over ghosts
    for ghost in state.getGhostPositions():
        # manhattan Distance to current ghost
        cur = min(util.manhattanDistance(pac, ghost), minG)
        #update minG if needed
        if cur < minG:
            minG = cur
    #add ghost feature
    features["ghost"] = minG
