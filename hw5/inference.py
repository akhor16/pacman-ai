...
class ExactInference(InferenceModule):
    # is written by help of internet resources
    """
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            #handling situation when it is jail position, so possibility is 1
            allPossible[self.getJailPosition()] = 1
        else:
            #it is not jail position, calculate every possible position and probability for them
            for p in self.legalPositions:
                #manhattan distance for each legal position to the pacman
                trueDistance = util.manhattanDistance(p, pacmanPosition)

                #probability for legal position
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        #iterate over legal positions
        for oldPos in self.legalPositions:
            # calculating new postion distribution
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            #iterating over all possible positions
            for newPos, prob in newPosDist.items():
                # calculate probabilities in current new position, here is += because
                # we can come here from different ways
                counter[newPos] += self.beliefs[oldPos] * prob
...
class ParticleFilter(InferenceModule):
    # Question 4 and 5 are written by help of internet resources
    """
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            #iterate over all legal positions
            for p in self.legalPositions:
                #calculating manhattan distance between given legal position and pacman
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    self.allPossible[p] = emissionModel[trueDistance] * self.beliefs[p]

            #normalization
            self.allPossible.normalize()
            #update beliefs
            self.beliefs = self.allPossible

            if self.allPossible.totalCount() == 0:
                self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        #iterate over all legal positions
        for oldPos in self.legalPositions:
            #getting position distribution
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            #iterate over new possible positions
...
class JointParticleFilter:
    # This class is written by help of internet resources
    """
